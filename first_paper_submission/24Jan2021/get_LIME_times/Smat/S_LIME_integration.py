...
S_mat_contents = loadmat('./S.mat')
S = S_mat_contents['S']
S_T = S.T # word embeddings shape num_words x 250
...
class Text2Embed(TransformerMixin):
    ...
    def transform(self, new_corpus=None, y=None):
        ...
        """
        full_matrix = get_text_vectors(S_T, vocabulary_dict, new_corpus)
...
S_embeddings = np.load('tweet_embed_ST.npy')
print('loaded numpy S embeddings')

# if these don't match the prior bert embeddings I'm working with, there's an issue
if not np.array_equal(embedded_tweets, S_embeddings):
	print('!!!!! WARNING: embedded_tweets do not match S embeddings file !!!!!')


# load trained classification algorithm
svc = joblib.load('svm_S_Fold 3.pkl')
...
# save explanations
with open('s_explanations.pkl', 'wb') as f:
    pickle.dump(explanations, f)

# save explanations as lists
with open('s_explanations_aslist.pkl', 'wb') as f:
	pickle.dump(explanation_tuples, f)

# save LIME explanation time
with open('s_lime_time.pkl', 'wb') as f:
    pickle.dump(lime_time, f)
